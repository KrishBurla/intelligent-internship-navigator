...
export default function ProfileView({ onOpenResumeModal, onRetakeQuiz }) {
    const [profile, setProfile] = useState({ name: '', email: '', education: '', field_of_study: '', skills: '', preference_tags: '' });
    ...
    const preferencesArray = profile.preference_tags ? profile.preference_tags.split(',').map(s => s.trim().replace(/-/g, ' ')).filter(Boolean) : [];
    ...
    const fetchProfile = useCallback(async () => {
        ...
        try {
            ...
            const response = await fetch(`${API_BASE_URL}/api/profile?email=${email}`, { headers: { 'Authorization': `Bearer ${token}` } });
                    ...
                    <div className="mt-2 p-4 border border-gray-200 rounded-lg">
                        ...
                        <div className="flex mt-4 relative">
                            ...
                            <input 
                                ...
                                onKeyDown={(e) => {
                                    if(e.key === 'Enter'){ 
                                        e.preventDefault(); // FIX: Prevents form submission
                                        handleAddSkill(e);
                                    }
                                }} 
